...
class MealPreparationController extends Controller
{
    ...
    public function updateStatus(Request $request, $id): JsonResponse
    {
        ...
        // Validation: Cannot cancel if order is not in Pending status
        if ($request->statue == OrderStatus::CANCELLED->value && $order->statue !== OrderStatus::PENDING->value) {
            return response()->json([
                'success' => false,
                'message' => 'Cannot cancel order. Only orders with Pending status can be cancelled.'
            ], 422);
        }

        // Validation: Cannot modify order if already Delivered or Cancelled
        if (in_array($order->statue, [OrderStatus::DELIVERED->value, OrderStatus::CANCELLED->value])) {
            return response()->json([
                'success' => false,
                'message' => 'Cannot modify order. Orders that are Delivered or Cancelled cannot be modified.'
            ], 422);
        }
